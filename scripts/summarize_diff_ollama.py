...
# ------------------------------
# OLLAMA SETTINGS (env-overridable)
# ------------------------------
...
def ping_ollama() -> bool:
    """
    Quickly verify the Ollama endpoint is reachable.
    """
    try:
        r = requests.get(f"{OLLAMA_ENDPOINT}/api/tags", timeout=5)
        r.raise_for_status()
        logging.info("Ollama reachable at %s", OLLAMA_ENDPOINT)
        return True
    except Exception as e:
        logging.error("Ollama not reachable at %s: %s", OLLAMA_ENDPOINT, e)
        return False
...
# ------------------------------
# Compute/Load Patch Helper
# ------------------------------
def load_or_compute_patch_bytes(args) -> bytes:
    """
    Returns unified diff bytes. If --patch provided, reads the file.
    Otherwise computes via git using provided SHA/base SHA or defaults.
    """
    if args.patch:
    ...
    # Compute diff via git (runner must have the repo checked out)
    if args.base_sha and args.sha:
        diff_range = f"{args.base_sha}..{args.sha}"
    elif args.sha:
        diff_range = f"{args.sha}~1..{args.sha}"
    else:
        diff_range = "HEAD~1..HEAD"

    logging.info("No --patch provided; computing diff for range: %s", diff_range)
    try:
        diff_bytes = subprocess.check_output(
            ["git", "diff", "--unified=3", diff_range],
            stderr=subprocess.STDOUT
        )
        # also write a copy to patch.diff for reference
        Path("patch.diff").write_bytes(diff_bytes)
        logging.info("Computed diff (%d bytes), saved to patch.diff", len(diff_bytes))
        return diff_bytes
    except subprocess.CalledProcessError as e:
        logging.exception("Git diff failed: %s", e.output.decode("utf-8", errors="replace"))
        raise
    except Exception as e:
        logging.exception("Failed to compute diff via git: %s", e)
        raise
...
def main():
    ...
    parser.add_argument("--patch", type=str, help="Path to patch file (unified Git diff)")
    ...
    # Optional args (accepted to align with workflow calls; not strictly needed)
    parser.add_argument("--repo", type=str, help="owner/repo (optional; ignored for local git diff)")
    parser.add_argument("--branch", type=str, help="branch name (optional; ignored for local git diff)")
    parser.add_argument("--sha", type=str, help="commit sha or PR head sha (optional)")
    parser.add_argument("--base-sha", type=str, help="base sha for range diffs (optional)")
    ...
    if args.save_json:
        # pick a safe JSON filename
        json_name = "patch.structured.json" if args.overwrite else f"patch_{time.strftime('%Y%m%d_%H%M%S')}.structured.json"
